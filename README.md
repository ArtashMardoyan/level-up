...
- **Multiple tracks** — pick a course from the landing page (Backend, Frontend, DevOps, QA today; Node.js, Go, React, Next.js marked "Coming soon" until filled in).
...
- **Text-to-speech** — read any question or answer aloud (browser `speechSynthesis`, with a voice picker in Settings), or use the course-wide audio player to auto-play through a whole module or the entire course, hands-free.
...
- **Light/dark theme** — toggle from the Settings panel; respects system preference on first load.
...
Each course's questions live in their own JSON file under `src/data/courses/`. A course shows up as active on the landing page as soon as its file has at least one entry — otherwise it's shown as "Coming soon".
...
- `id` must be unique and stable within the file — favorites and reviewed-progress are keyed by it.
