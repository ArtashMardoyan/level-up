...
export function useReviewState(courseId) {
  ...
  const toggleFavorite = useCallback(
    (id) => {
      setState((prev) => {
        ...
        const favorites = isFav ? prev.favorites.filter((x) => x !== id) : [...prev.favorites, id]
  ...
  const toggleReviewed = useCallback(
    (id) => {
      setState((prev) => {
        ...
        const reviewed = isReviewed ? prev.reviewed.filter((x) => x !== id) : [...prev.reviewed, id]
  ...
  return { toggleFavorite, toggleReviewed, markReviewed, state }
}
