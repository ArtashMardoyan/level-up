...
;(() => {
  ...
  var __defNormalProp = (obj, key, value) =>
    key in obj
      ? __defProp(obj, key, { configurable: true, enumerable: true, writable: true, value })
      : (obj[key] = value)
  ...
  function parseDcDocument(doc) {
    ...
    const { preview, props } = parseDataProps(scriptEl?.getAttribute('data-props') ?? null)
    return {
  ...
  function parseDataProps(raw) {
    if (!raw) return { preview: null, props: null }
    let parsed
    try {
      parsed = JSON.parse(raw)
    } catch {
      return { preview: null, props: null }
    }
  ...
  function collectProps(node, kind, host) {
    ...
    return { pseudoClasses, propGetters, hintSize }
  }
          ...
          if (v === void 0) {
            if (!ctx?.__streamingNow) {
              if (document.body?.hasAttribute('data-dc-editor-on')) {
                return h('span', { className: 'sc-interp sc-unresolved', key: i }, '{{ ' + p.trim() + ' }}')
              }
              warnUnresolved(ctx, '{{ ' + p.trim() + ' }} never resolved \u2014 rendered as empty')
              return null
            }
            return h('span', { className: 'sc-interp sc-missing', key: i }, p.trim())
          }
          if (getReact().isValidElement(v) || Array.isArray(v)) {
            return h(getReact().Fragment, { key: i }, v)
          }
          if (v === null || typeof v === 'boolean') return null
          return h('span', { className: 'sc-interp', key: i }, String(v))
        })
      )
  }
  ...
  function walkElement(el, host) {
    ...
    const { pseudoClasses, propGetters } = collectProps(el, 'dom', host)
    const kids = walkChildren(el, host)
    return (vals, ctx, key) => {
      const props = {
        key: key + keySuffix,
  ...
  function Placeholder({ streaming, hintSize, error, name }) {
    const [w, hgt] = (hintSize || '100%,60px').split(',')
    return h(
      'div',
      {
        className: 'sc-placeholder' + (streaming ? ' sc-streaming' : ''),
        style: { height: hgt && hgt.trim(), width: w.trim() },
        title: name
      },
  ...
  function hintToMin(hint) {
    ...
    return { minHeight: hgt && hgt.trim(), minWidth: w.trim() }
  }
  function createComponentFactory(registry, ensureFetched) {
    ...
    class StreamableComponent extends React.Component {
       ...
       *  wiring stripped. */
      __userProps() {
        const { __hostStyle, __hintSize, __tplId, __name, ...rest } = this.props
        return rest
      }
      ...
      render() {
        ...
        if (!r.tpl) {
          return h('div', hostBase, h(Placeholder, { hintSize: this.props.__hintSize, name: this.__name }))
        }
  ...
  function cdnScriptFor(url, sri) {
    ...
    return typeof v === 'string' && v ? { src: v } : { integrity: sri, src: url }
  }
  ...
  function createExternalModules(onResolved) {
    ...
    return { resolve: resolve2, resolveGlobal, getError, load }
  }
  ...
  function createHelmetManager(doc, isStreaming) {
    ...
    return { setDesignDocMode, compile }
  }
  ...
  function createRuntime(doc = document) {
    ...
    function dcUpdate(name, kind, content, streaming) {
      ...
      if (kind === 'html') {
        ...
        const { preview, props } = parseDataProps(content)
        const r = registry.get(name)
