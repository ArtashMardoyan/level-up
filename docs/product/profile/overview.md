...
5. **Saved questions** — **реально**: сами избранные вопросы из `GET /progress/saved?lang=`
   (кликабельный список → `#slug/ref`), а не только счётчики по курсам.
...
- **Серия (streak)** — ✅ реально: `GET /progress/summary` → `currentStreak` (поле есть на `users`).
- **Recent activity** — ✅ реально: лента из последних уведомлений (`GET /notifications`) + недавних интервью.
- **Список избранных вопросов** — ✅ реально: `GET /progress/saved?lang=` возвращает сами избранные
  вопросы (курс slug/title/accent + ref/module/текст, en-fallback); список кликабельный → `#slug/ref`.
