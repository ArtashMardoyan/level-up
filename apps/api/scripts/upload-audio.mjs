...
//
// 1. Syncs audio/courses/**/*.mp3 to s3://$S3_BUCKET/audio/courses/, with
//    --delete so S3 files no longer present locally (e.g. the old per-phase
//    files) are removed.
// 2. Stamps each uploaded file's S3 key into the matching question in
//    internal/seed/data/<id>/<lang>.json under `audio` (a single key string).
//
...
const localDir = path.join(scriptDir, '..', 'audio', 'courses')
const coursesDir = path.join(scriptDir, '..', 'internal', 'seed', 'data')
...
if (!existsSync(localDir)) {
  console.log('Nothing to upload — run generate-audio.mjs first (audio/courses/ is empty).')
  process.exit(0)
}
