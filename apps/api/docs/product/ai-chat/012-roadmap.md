...
- **Scope:** `docs/product/ai-chat/**`. No production code.
- **Risks:** none (docs only).
- **Dependencies:** read access to both repos + `docs/product/interview/**`.
...
- **Risks:** partial-JSON rendering if JSON-object mode is kept (mitigation: stream prose, not JSON —
  ...
  `docs/product/interview/STATUS.md` must be re-validated live).
...
- **Scope:** compose Phases 5+7 into the complete interview flow (spoken questions, spoken answers,
  ...
  `docs/product/interview/015`); persistence parity with the text interview so History/Results/Reports work
...
- **Dependencies:** all prior phases; product spec from `docs/product/interview/**`.
