...
The AI Chat is **shipped and live** as the AI Interview Coach (see `docs/interview/STATUS.md`):
...
- **The chat's only user-visible AI prose is the next-question bubble** (`reaction` + `question`).
  ...
  Results screen — `docs/interview/STATUS.md`). So token-by-token streaming maps cleanly onto the
...
- **Spec:** read this folder `001`–`014` **and** the product spec `docs/interview/**` (they are
